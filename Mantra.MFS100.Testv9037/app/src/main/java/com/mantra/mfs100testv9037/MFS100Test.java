...
import com.mantra.mfs100testv9037.R;
...
public class MFS100Test extends Activity implements MFS100Event {
    ...
    public void FindFormControls() {
        ...
        cbLiveDetection = (CheckBox) findViewById(R.id.cbLiveDetection);
        cbLiveDetection.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                detectLiveness = isChecked;
                if(!isCaptureRunning){
                    InitScanner();
                }
            }
    ...
    private void InitScanner() {
        try {
            int ret = mfs100.Init(detectLiveness);
            if (ret != 0) {
    ...
    private void WriteFile(String filename, byte[] bytes) {
        try {
            String path = MFS100Test.this.getExternalFilesDir(null) +
                     "//FingerData";
    ...
    public void OnDeviceAttached(int vid, int pid, boolean hasPermission) {
        ...
        if (vid == 1204 || vid == 11279) {
            if (pid == 34323) {
                ...
                ret = mfs100.Init(detectLiveness);
                if (ret == 0) {
                    showSuccessLog(key);
                } else {
                    SetTextOnUIThread(mfs100.GetErrorMsg(ret));
    ...
    @Override
    public void OnMFS100Preview(byte[] rawData, int quality, String previewInfo) {
        SetTextOnUIThread(previewInfo);

        try {
            if (quality != 0 && rawData != null) {
                Bitmap previewBitmap = BitmapFactory.decodeByteArray(rawData, 0,
                        rawData.length);
                MFS100Test.this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
